...
df = df.dropna(subset=['Anzahl befüllter Felder', 'Besuche'])
...
avg = df.groupby('Anzahl befüllter Felder')['Besuche'].mean().reset_index()

# 📊 Plot: Durchschnittswerte + Modellvorhersage
plt.figure(figsize=(10,6))
plt.scatter(avg['Anzahl befüllter Felder'], avg['Besuche'], label="Durchschnittswerte", alpha=0.7)
plt.plot(df['Anzahl befüllter Felder'], df['mu_hat'], 'r.', alpha=0.2, label="NB-Vorhersagen")
plt.title("Negative-Binomial-Regression: Anzahl befüllter Felder ~ Besuche")
plt.xlabel("Anzahl befüllter Felder")
plt.ylabel("Besuche")
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.savefig("NB_Anzahl_befüllter_Felder_vs_Besuche.png")
