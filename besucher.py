...
df = df.dropna(subset=['Anzahl Photos', 'Besucher'])
...
avg = df.groupby('Anzahl Photos')['Besucher'].mean().reset_index()

# 📊 Plot: Durchschnittswerte + Modellvorhersage
plt.figure(figsize=(10,6))
plt.scatter(avg['Anzahl Photos'], avg['Besucher'], label="Durchschnittswerte", alpha=0.7)
plt.plot(df['Anzahl Photos'], df['mu_hat'], 'r.', alpha=0.2, label="NB-Vorhersagen")
plt.title("Negative-Binomial-Regression: Anzahl Photos ~ Besucher")
plt.xlabel("Anzahl Photos")
plt.ylabel("Besucher")
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.savefig("NB_Besucher_vs_Photos.png")
...
plt.grid(True)
plt.tight_layout()
plt.savefig("NB_Besucher_vs_Photos_zoom.png")
